...
trait Lifter(serializeQuats: Boolean) {
  ...
  given liftableQuatProduct: NiceLiftable[Quat.Product] with {
    def lift =
      ...
      case Quat.Product.WithRenamesCompact(tpe, fields, values, renamesFrom, renamesTo) => '{ io.getquill.quat.Quat.Product.WithRenamesCompact.apply(${tpe.expr})(${fields.toList.expr}: _*)(${values.toList.expr}: _*)(${renamesFrom.toList.expr}: _*)(${renamesTo.toList.expr}: _*) }
  }
